...
public class PhysicalDBPool {
	...
	public static final int BALANCE_ALL_BACK_MAN = 11;
	public static final int BALANCE_ALL_READ_MAN = 33;
...
//		}
	...
	public void getRWBanlanceCon(String schema, boolean autocommit,
		...
		switch (banlance) {
			...
			/**modified by lixiliang   no auto use default master when slave down **/
			case BALANCE_ALL_BACK_MAN: {
				// all read nodes and the standard by masters
				okSources = getAllActiveRWSources(true, false, checkSlaveSynStatus());
				theNode = randomSelectNoDefaultWhenEmpty(okSources);
				break;
			}
			case BALANCE_ALL_READ_MAN: {
				okSources = getAllActiveRWSources(false, false, checkSlaveSynStatus());
				theNode = randomSelectNoDefaultWhenEmpty(okSources);
				break;
			}
		...
		//add by lixiliang
		if(null==theNode){
			LOGGER.warn("no valid read source ,return null ");
			throw new IOException("no valid read datasource connection");
		}
		if (LOGGER.isDebugEnabled()) {
...
//					LOGGER.debug("allSlaves.length i:::::: " + i);
					LOGGER.debug("allSlaves.length :::::: " + allSlaves.length);
					if (isAlive(slave)) {
...
//					index = Math.abs(random.nextInt()) % readSources.size();
	...
	/**
	 * 区别于 randomSelect，参数为空时默认返回null对象
	 * add by lixiliang 2017-08-18
	 * @param okSources
	 * @return
	 */
	public PhysicalDatasource randomSelectNoDefaultWhenEmpty(ArrayList<PhysicalDatasource> okSources) {
		if (okSources.isEmpty()) {
			return null;
		}
		int length = okSources.size(); 	// 总个数
